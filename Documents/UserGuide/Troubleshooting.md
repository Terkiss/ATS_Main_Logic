...
### ❓ Q6: "HMAC Verification Failed" 에러와 함께 연결이 즉시 끊겨요.
**증상:** 클라이언트가 패킷을 보내자마자 서버에서 연결을 강제로 종료(Sanction)합니다.
**원인:** 패킷의 무결성을 증명하는 HMAC-SHA256 서명이 일치하지 않기 때문입니다. 클라이언트와 서버 간의 **Secret Key**가 다르거나, 전송 중 데이터가 변조되었을 가능성이 있습니다.
**해결책:** 
1. 클라이언트 SDK 설정에서 사용 중인 `SecretKey`가 서버의 `config.txt`에 설정된 값과 일치하는지 확인하세요.
2. 패킷을 직접 구성하는 경우, 페이로드의 마지막 부분에 올바른 HMAC 서명이 포함되었는지 점검하세요.