...
public enum ProtocolSelect : byte
{
    ...
    JoinGroupProtocol = 8,
    RpcProtocol = 100,       // 범용 추상화 RPC 프로토콜
    QueueCountCommand = 101, // 큐 상태 조회
    ImageDumpCommand = 102   // 이미지 바이너리 덤프
}
```
...
`Direct` 방식의 보안 프로토콜(예: `ImageDumpCommand`, `UdpRegisterProtocol`)을 서버로 보낼 때는, 페이로드 앞에 토큰 길이와 토큰 문자열을 추가로 삽입해야 합니다.

- **구조**: `[SendType(1)][ProtocolSelect(1)][TokenLength(4)][TokenBytes(N)][Payload(M)]`
